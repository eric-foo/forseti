...
Before any online fetch or search intended to capture or preserve a source as evidence for an Orca decision, route through the Source Capture Armory runbook (`orca-harness/docs/source_capture_agent_runbook.md`) as the source of truth for that walking; generic `WebSearch`/`WebFetch` is not the sanctioned source-capture path, and ordinary non-evidence lookups stay out of scope. The owning rule is `.agents/workflow-overlay/source-loading.md` → Online Source-Capture Front Door.
