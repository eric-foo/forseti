...
open_next:
  ...
  - forseti-harness/signal_content/models.py
...
input_pins:
  ...
  forseti-harness/signal_content/models.py: committed 834c930 (the v0 model derived INTO; 17 model tests green)
  forseti-harness/ecr/ (SP-6 source-visibility deriver): committed 6329d71 (the pattern reused)
...
Mirroring `forseti-harness/tests/unit/test_signal_content_models.py` (17 green) and the `ecr/` deriver tests, the future deriver build (not authorized here) must satisfy tests able to FAIL:
...
direction_change_propagation:
  ...
  evidence:
    ...
    - forseti-harness/signal_content/models.py (834c930) — the v0 model + the signal_event_time honesty gap
...
- Sources (pins verified by the panel's fresh reads): `core_spine_v0_signal_content_record_architecture_v0.md` (834c930, controlling), `forseti-harness/signal_content/models.py` + `__init__.py` (834c930; 17 model tests green), `forseti-harness/ecr/` (deriver/models/__init__; SP-6 at 6329d71 — the reused pattern), `jsg01_sp6_source_visibility_derivation_architecture_plan_v0.md` (the precedent shape), `forseti-harness/source_capture/models.py` (`VisibleFact`, `PacketTiming`, packet/slice keys), `core_spine_v0_data_and_cleaning_spine_boundary_v0.md` (SP-5 decision B `:303`; JSG-01 frozen; reserved EvidenceUnit architecture), `forseti-harness/tests/unit/test_signal_content_models.py` (the validation discipline). Decisive **negative** finding (all three lenses, grep-confirmed): no authored-classification input record and no SP-5 finalizer mechanism exist anywhere in the harness.
