...
relates_to:
  conductor_predicate: forseti/product/spines/judgment/conductor/judgment_quality_promotion_operating_model_v0.md  # JSG-01 row (:209) — read, never edit
  ratified_field_schema: forseti/product/spines/foundation/product_contract/core_spine_v0_data_and_cleaning_spine_boundary_v0.md  # SP-1/2/3/6 ratification + decisions B/C; reserved decisions
  ecr_frame: forseti/product/spines/ecr/evidence_candidate_record/ecr_consolidation_v0_frame_source_visibility_slice_architecture_plan_v0.md  # INV-1..5, three-mode binding rule (inherited verbatim)
...
built_inputs: >
  ECR derivers: forseti-harness/ecr/deriver.py (SP-1/2/3/6, pure, bind no EvidenceUnit).
  SP-5 model + validate-only consumer: forseti-harness/schemas/finalization_models.py (committed a37f896).
  SP-5 producer (acting half): forseti-harness/runners/run_finalization_receipt.py (built under the same
  commission; review/adjudication state must be checked in that lane, not inferred here).
  EvidenceUnit: forseti-harness/schemas/case_models.py:53-73 (UNCHANGED by this slice).
