...
| --- | --- | --- | --- | --- |
| JSG-01 | data-capture obligation contract + core-spine boundary own the source-identity obligation and declare the **ratified** JSG-01 source-side ECR field schema (SP-1/2/3/6) plus the **ratified JSG-01-scoped `EvidenceUnit` binding contract** (2026-06-12); packing interface owns `pre_decision_status` | packing final status + ECR source-side postures + `FinalizationReceipt`, composed onto the case packet's bound evidence unit by `orca-harness/evidence_binding/` | `pre_decision_status` present and set to an allowed *cleared* pre-decision value (not `excluded` or an uncertain/placeholder value), read through the SP-5 finalization chain. The finalization-provenance subpredicate — that the final `pre_decision_status` was finalized by the Judgment authority (AR-01, **resolved** — operator-for-now, a distinct cross-family act; see `docs/decisions/ar_01_pre_decision_status_finalizer_staffing_v0.md`) — evaluates via the validate-only consumer (`orca-harness/schemas/finalization_models.py`, `a37f896`; producer acting half `orca-harness/runners/run_finalization_receipt.py`): cleared **only** on a single current, cross-family, binding-consistent `FinalizationReceipt` whose recorded `judge_model_family` matches the run's judge family — a mismatched or placeholder-recorded judge family **blocks** until a correction receipt re-points it (append-only). The source-identity, inspectability, timing/cutoff, and source-visibility subpredicates route to the **ratified** owner fields and allowed values (core-spine boundary: SP-1 `source_identity_state` ∈ {`resolved`, `family_only`}; SP-2 `inspectability_state == inspectable_verifiable`; SP-3 timing `== pre_cutoff` carried over the producer's `PacketTiming.cutoff_posture`; SP-6 `source_visibility_posture` per the ratified decision-C grade) and **evaluate determinately** via the built derivers (`orca-harness/ecr/deriver.py`) composed onto the bound case-packet evidence unit by the ratified binding (`orca-harness/evidence_binding/`; SP-2/SP-3 read at the bound slice's row; no aggregate verdict is composed — this conductor combines the subpredicates). Per the unfreeze act's D2 consumption (option b): D2 stays deferred; the corroborated SP-6 tier is unreachable and archive+current packets land on named residuals — residuals do **not** clear (conservative; never a pass) | **UNFROZEN** (owner dated act 2026-06-12 — `docs/decisions/jsg01_unfreeze_decision_v0.md`; first carrying packet `orca-harness/cases/product_learning/jsg01_binding_assembly_proof_v0`): evaluable over a bound case packet; any subpredicate whose owner field is absent or unresolvable routes to not-cleared per Seam 3 (missing evidence is not a pass). **JSG-01 clears no case until a separately authorized run evaluates one** |
