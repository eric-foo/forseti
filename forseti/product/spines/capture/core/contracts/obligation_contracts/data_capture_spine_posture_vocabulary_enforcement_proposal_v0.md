...
stale_if:
  ...
  - forseti-harness/source_capture/models.py or source_quality.py changes the posture fields, VisibleFact model, or _source_time fallback.
```
...
- existing tests/packets store capture-context strings in it — e.g.
  `known_fact("local cutoff posture")` (`forseti-harness/tests/unit/test_source_capture_packet.py:324-330`)
  and `"local JSON/file state supplied…"` /
  `"local markdown artifact state supplied…"`
  (`forseti-harness/reports/source_capture/slot3_reddit_batch1_packet_dry_run_2/manifest.json:153-156`,
  `…_post_patch_dry_run/manifest.json:151-154`);
- `source_quality.py` reads it as a **time fallback**
  (`_source_time` → `cutoff_posture`, `forseti-harness/source_capture/source_quality.py:434-442`).
...
- `forseti-harness/source_capture/models.py` — schema enforcement.
- `forseti-harness/source_capture/source_quality.py` — **`_source_time` must not
...
- `forseti-harness/source_capture/packet_assembly.py` — posture-honesty validator
