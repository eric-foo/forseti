...
| Kohl's | No current capture route is admitted. Direct HTTP returned 403 on the commissioned Tower 28 LipSoftie PDP and Kohl's out-of-country FAQ. Anonymous CloakBrowser rendered Akamai `Access Denied` both cold and after a successful humanized homepage warm-up; the disproven warm-up adapter/CLI route was removed. | `logged_out_public`; no profile, storage state, cookie injection, credential, login, cart, or delivery mutation. No proxy was loaded because no registered US residential proxy profile was available. | `UNKNOWN_REQUIRED_ACCESS_BLOCKED` | `UNKNOWN_REQUIRED_ACCESS_BLOCKED` | `UNPINNED` | Direct PDP `01KXT0245PZBHZSYJHM5376BCA`; cold browser PDP `01KXT04HA0TT33RH7BAWQ38H58`; Direct FAQ `01KXT09ERZ6584J7M4J07WS706`; warmed policy/PDP `01KXT3432PEF0NXEZE0VWEWMMD` / `01KXT38WKZMXVMMY18CDX3SC66`; all captured 2026-07-18 UTC with requested/final URLs, raw SHA-256 values, and byte lengths fresh-verified. | `.com`, a dollar glyph, or a US proxy exit is not pin evidence. The warmed packets falsify cold deep-linking and disabled humanization as sufficient explanations, but do not distinguish exit-IP reputation from browser/TLS fingerprinting. Re-probe only with a registered US residential profile whose metadata binds US geo-IP, `en-US`, and a US timezone; admission still requires a preserved retailer-owned US policy signal and a separate bound product offer with exact `USD`. |
