...
| Kohl's | No current capture route is admitted. Direct HTTP on the commissioned Tower 28 LipSoftie PDP and Kohl's out-of-country FAQ returned HTTP 403; anonymous CloakBrowser on the PDP rendered an Akamai `Access Denied` page. | `logged_out_public`; no profile, storage state, cookie injection, credential, login, cart, or delivery mutation. No proxy was loaded because no registered US residential proxy profile was available. | `UNKNOWN_REQUIRED_ACCESS_BLOCKED` | `UNKNOWN_REQUIRED_ACCESS_BLOCKED` | `UNPINNED` | PDP Direct HTTP `01KXT0245PZBHZSYJHM5376BCA`; PDP CloakBrowser `01KXT04HA0TT33RH7BAWQ38H58`; FAQ Direct HTTP `01KXT09ERZ6584J7M4J07WS706`; all captured 2026-07-18 UTC with requested/final URLs, raw SHA-256 values, and byte lengths fresh-verified. | `.com`, a dollar glyph, or a US proxy exit is not pin evidence. Re-probe only with a registered US residential profile whose metadata binds US geo-IP, `en-US`, and a US timezone; admission still requires the bound Tower 28 product, exact `USD`, and an explicit retailer-owned US signal on the preserved page. The generic browser access classifier recorded `access_blocked=false` despite literal Akamai `Access Denied`; the subject/USD sufficiency gate nevertheless failed closed. |
