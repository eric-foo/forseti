...
scope: >
  Stabilizes the Amazon, Sephora, Luckyscent, and Ulta Retail/PDP projection slice as a
...
use_when:
  ...
  - Checking what Amazon, Sephora, Luckyscent, and Ulta PDP projection may carry from raw packets and what it must residualize.
...
stale_if:
  ...
  - A new Amazon, Sephora, Luckyscent, or Ulta capture recon verdict changes the target substrate or access posture.
...
`sephora_pdp_aggregate` and `luckyscent_pdp_aggregate` routes default to
retailer-owned parser-versioned content records after their market-pin and all
...
| Preserved bytes | The helper needs hash-verified bytes for every file it consumes. A valid Sephora or Luckyscent content record is preferred when present; otherwise missing raw bytes are a hard error, not an empty projection. |
| Rendered DOM | Raw, sample, legacy, and unflipped routes preserve the rendered DOM. A successful Sephora or Luckyscent content packet may omit it after recording its hash and byte count; failed admission preserves it. |
