...
| Retail/PDP packet/projection | `forseti-harness/runners/run_source_capture_cloakbrowser_packet.py --source-family retail_pdp`; `run_retail_pdp_projection.py`; retailer-owned parser-fit checkers `run_sephora_pdp_parser_fit_check.py` and `run_luckyscent_pdp_parser_fit_check.py`; `forseti-harness/source_capture/retail_pdp_projection.py` | `sephora_pdp_aggregate` and `luckyscent_pdp_aggregate` default to family-owned content mode only after their respective US/USD market, access, sufficiency, and projection gates pass. Luckyscent retains all three target variants and all eight rendered target reviews in one offer row and one review-substrate row. Raw/sample modes and packet-directory projection remain for drift checks and legacy packets. Every other retail profile remains raw. No live broad crawl, scheduler, ECR, Cleaning, or Judgment. |
