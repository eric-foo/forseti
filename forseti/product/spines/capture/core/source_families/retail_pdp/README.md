...
| Retail/PDP packet/projection | `forseti-harness/runners/run_source_capture_cloakbrowser_packet.py --source-family retail_pdp`; `run_retail_pdp_projection.py`; the Sephora and Nordstrom parser-fit runners; `forseti-harness/source_capture/retail_pdp_projection.py` | `sephora_pdp_aggregate` and `nordstrom_pdp_aggregate` default to family-owned content mode after their retailer-owned US/USD, access, sufficiency, and projection gates pass; raw/sample modes and packet-directory projection remain for drift checks and legacy packets. Every other retail profile remains raw. Nordstrom's visible shipping destination stays an unpinned residual, not US-delivery proof. No live broad crawl, scheduler, ECR, Cleaning, or Judgment. |
