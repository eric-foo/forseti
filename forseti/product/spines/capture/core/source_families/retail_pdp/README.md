...
| Retail/PDP packet/content | `run_source_capture_cloakbrowser_packet.py`; `source_capture/retail_pdp_content.py`; `cleaning/retail_pdp.py` | The four admitted profiles default to canonical content after retailer-owned pin/access/sufficiency/extraction gates. Raw remains the failure and unflipped-route posture. |
| Sephora brand-grid packet/projection | `run_source_capture_cloakbrowser_packet.py`; `source_capture/sephora_brand_grid.py`; `source_capture/retail_grid_projection.py`; `docs/research/forseti_sephora_brand_grid_capture_live_proof_v0.md` | Raw remains authoritative. The separate typed parent-product view must reconcile subject, page-declared count, serialized placements, unique parents, and termination. US/USD admission independently requires retailer-owned country and explicit currency evidence; the first Summer Fridays proof reconciled 34 of 34 products but remained market-blocked. |
