...
| Initial content-onboarding completion | Creator Registry materialized onboarding projection over verified committed Bronze, mirrored into profile-current | Add one account-scoped state/evidence block using stable account ids and exact packet lineage; refresh centrally after capture | Let individual runners manually flip state, treat RSS/discovery/staging as completion, or conflate onboarding with freshness or quality |
...
- some enforcement remains prose-first and checkpoint-driven rather than
  mechanical; two review findings or patch prompts in the same failure class,
  one scan/capture lane needing to manually re-derive the same routing rule, or
  one preflight/receipt mismatch that a deterministic check could catch is the
  concrete upgrade trigger for a validator, runner gate, or generated check; and
