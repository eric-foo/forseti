...
only in gitignored `forseti-harness/_test_runs/`; probe scripts are disposable, uncommitted
`forseti-harness/_scratch/` throwaways. This doc was **corrected once mid-investigation** — see
...
Raw capture (gitignored, disposable) under `forseti-harness/_test_runs/`:
...
under `forseti-harness/_scratch/`: `reel_viewcount_probe.py`, `reel_cursor_depth_probe.py`
