...
Raw evidence (gitignored, not committed): `forseti-harness/_test_runs/related_accounts/<handle>/`
(`web_profile_info.json`, `profile.html`, `sniffed_responses.jsonl`, `summary.json`) +
`aggregate.json`.

## Method (the widened substrate net)

Disposable logged-out Playwright probe (`forseti-harness/_scratch/related_accounts_probe.py`).
...
Raw: `forseti-harness/_test_runs/related_snowball/{nodes.jsonl, summary.json}`.
