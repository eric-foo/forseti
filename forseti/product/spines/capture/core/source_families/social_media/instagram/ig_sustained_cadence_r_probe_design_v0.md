...
branch_or_commit: designed against origin/main @ f266f83b (PR #154); instrument = forseti-harness/runners/run_source_capture_ig_calls_packet.py at that revision.
```
...
Run from `forseti-harness/`. One invocation per (creator, round); unique `--output`
