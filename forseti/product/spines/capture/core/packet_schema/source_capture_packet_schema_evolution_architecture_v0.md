...
open_next:
  ...
  - forseti/product/spines/capture/core/source_capture_toolbox/source_capture_packet_fixture_admission_criteria_v0.md
input_hashes:
  forseti-harness/source_capture/models.py: 3B89A19BAEAB90762C34FE0C95517A005D383933E48057C1ADD12E004A3A7245
...
stale_if:
  ...
  - forseti-harness/source_capture/models.py SHA256 drifts from the pinned input_hash above.
```
...
external_source_boundary: agent-workflow reusable mechanics only; jb is NOT Forseti authority.
...
`forseti-harness/source_capture/models.py` SHA256 =
...
| --- | --- |
| `forseti-harness/source_capture/models.py` | `SourceCapturePacket(StrictModel)`; `StrictModel` = `extra="forbid"` (`schemas/case_models.py:11`). `manifest_version` is a plain `str` (`models.py:162`) with **no validator and no reader**. `SOURCE_CAPTURE_MANIFEST_VERSION="source_capture_packet_manifest_v1"` (`models.py:12`). R2 closed-vocab + `hash_basis` validators present (`models.py:53-66,82-94,105-114,185-199`). |
| `forseti-harness/source_capture/source_quality.py` | **Only** production read-back: `SourceCapturePacket.model_validate(...)` at `:99` (strict; raises on a v0 packet). The multi-row census path `_assemble_source_quality_row` (`:509-525`) **already** wraps it in try/except → degrades to `manifest_uninspectable` + visible stop (coarse: lumps "old-but-honest" with "corrupt"). |
| `forseti-harness/source_capture/writer.py` | Stamps `manifest_version` + `obligation_contract_version` (`:134-135`); model construction (`:132`) = the write-time strict gate. Hard-codes `hash_basis="raw_stored_bytes"` (`:266-267`). |
...
| 3 v0 packets under `forseti-harness/reports/source_capture/slot3_reddit_batch1_*/manifest.json` | Declare `manifest_version: …_v0`; scratch dry-run evidence; **no live consumer**. (`dry_run` fails on `hash_basis` only; the other two also on `cutoff_posture`.) |
...
- Single production read-back (`source_quality.py:99`) verified by repo search
  within `forseti-harness`; readers outside `forseti-harness` were not enumerated
