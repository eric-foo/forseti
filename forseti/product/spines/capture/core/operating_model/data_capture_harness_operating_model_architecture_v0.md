...
| `.agents/workflow-overlay/source-of-truth.md` | Source hierarchy and conflict rules | Current user instruction, `AGENTS.md`, overlay, then Orca docs are controlling; `jb` rules are not Forseti authority | modified |
...
| `workflow-deep-thinking` method instructions | Procedural reasoning discipline only | Treat candidate architectures as candidates, compare options, preserve uncertainty and strict-claim boundaries | installed method copy; not Forseti authority |
| `workflow-architecture-planning` method instructions | Procedural architecture-planning discipline only | Standard profile with directional, adversarial, and grounding passes; target only if invariant, core/satellite split, non-goals, and blockers are clear | installed method copy; not Forseti authority |
