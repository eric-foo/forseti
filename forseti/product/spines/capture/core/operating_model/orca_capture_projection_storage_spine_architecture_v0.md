...
open_next:
  ...
  - forseti-harness/source_capture/models.py   # the packet model with the capture-shape gap
  - forseti-harness/capture_spine/   # the existing satellite/projection-transform idiom to extend
stale_if:
...
- It **extends, does not invent**, the satellite/projection-transform idiom that already ships in
  `forseti-harness/capture_spine/` (`linkedin_lane/`, `linkedin_live_adapter/`, `reddit_candidate_intake/`,
  …) and the typed-posture discipline already in `forseti-harness/source_capture/models.py` (`VisibleFact`).
...
`forseti-harness/source_capture/models.py` (`SourceCaptureSlice`).
...
- **Observed this session in the authoring environment (primary source; re-confirmable via the ledger
  ...
  | `capture_spine/` sibling idiom exists | `forseti-harness/capture_spine/` (origin/main `e0b939a2`) | `linkedin_lane/`, `linkedin_live_adapter/`, `reddit_candidate_intake/`, `linkedin_graph_frontier/`, `reddit_graph_frontier/` |
  | packet model has no typed metric field | `forseti-harness/source_capture/models.py` (ig-cadence-rails `c365dca2`) | `SOURCE_CAPTURE_MANIFEST_VERSION="source_capture_packet_manifest_v1"`; `VisibleFact{known,unknown_with_reason,not_attempted,not_applicable}`; `SourceCaptureSlice` carries no metric field |
