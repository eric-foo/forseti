# Creator Commercial Projection Calibration Deck v0

```yaml
retrieval_header_version: 1
artifact_role: Product calibration deck (required Creator Signal commercial-panel authoring companion)
scope: >
  Teaches a cold author how to turn evidence-backed creator and participating-
  audience signals into a specific, maximally commercial hiring proposition.
  It supplies a transformation method and calibration patterns, not creator
  claims, a closed taxonomy, or an alternative to the controlling contract.
use_when:
  - Creating or materially rewriting a buyer-facing commercial creator-fit panel.
  - Calibrating whether creator copy is specific and valuable enough to sell Forseti's data.
  - Dogfooding commercial projection on a creator not used as an example here.
...
stale_if:
  ...
  - Held-out dogfood repeatedly produces generic, copied, or unsupported hiring propositions.
```

## Status And Authority

`OWNER_DIRECTED_CALIBRATION_V0` — required authoring companion to
`creator_audience_triangulation_and_commercial_projection_v0.md`.

The controlling contract decides what the panel may claim. This deck teaches
how to discover and express the strongest commercial claim inside that ceiling.
Its examples calibrate reasoning and sentence strength; they are not evidence
for another creator and must never be copied as if they were.
...
direction_change_propagation:
  ...
  intentionally_not_updated:
    ...
    - path: .agents/workflow-overlay/source-loading.md
      reason: Targeted loading already follows a controlling artifact's open_next and use_when routes; no global product-specific rule belongs in the overlay.
    - path: AGENTS.md
      reason: The project kernel already routes product work through the spine and overlay; duplicating a product-specific companion rule would fork authority.
  ...
  non_claims:
    ...
    - not readiness
    - not buyer proof
    - not runtime or model reliability
```
