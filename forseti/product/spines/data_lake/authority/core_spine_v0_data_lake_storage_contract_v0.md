...
| Derived Result Store | Hold append-only lane-owned derived records keyed to a typed source anchor: normally raw, with an explicitly admitted capture-event anchor for a derived-first lane such as routine retail-grid monitoring. | Second raw source of truth, fake raw receipt, merged cross-kind blob, or rewritten or deleted derived history. | Deferred with derived-record physical-home blocker. |
...
direction_change_propagation:
  ...
  stale_language_search: >
    ...
    Attachment Record serialization" forseti/product/spines/data_lake/authority
