...
def test_fetch_with_retry_recovers_after_transient_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    # A read-phase timeout (now surfaced by direct_http as a TIMEOUT failure, not a crash) is
    # transient: the retry wrapper backs off and recovers instead of failing the rung on a
    # momentarily slow archive (observed live on the Wayback CDX during cross-archive capture).
    sleeps: list[float] = []
    calls = {"n": 0}
