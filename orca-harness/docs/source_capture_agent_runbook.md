...
- use unbuilt or separately gated methods from this runbook, including browser
  automation, APIs, SDKs, scraper frameworks, proxy/session behavior,
  anti-detect behavior, CAPTCHA solving, or commercial fetch services;
...
If the requested capture needs browser-rendered content, login-visible content,
JavaScript rendering, or screenshot preservation, stop with
`visible_capture_limitation`: the Honest Browser Snapshot adapter is not built
yet.
...
non-content shell, report `visible_capture_limitation` and name the unbuilt
Honest Browser Snapshot path. Do not describe the packet as a clean content
capture.
...
Direct HTTP, Media / Asset, and Archive.org runners have default source-family
...
| Browser-rendered or login-visible page | none yet | Stop; Honest Browser Snapshot adapter is not built. |
...
source_capture_agent_report:
  runner: <local_file|direct_http|media_asset|archive_org>
