...
The current dry-run path under `forseti-harness/reports/source_observability/` is
operator-chosen output, not a blessed durable fixture location.

Do not ignore, delete, move, or bless the whole `forseti-harness/reports/`
...
file patterns, not the entire `forseti-harness/reports/` tree. Pattern examples
...
The broader untracked `forseti-harness/reports/` state is a repo-health issue, not
