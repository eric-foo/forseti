"""Parser-fit drift check for pinned Sephora aggregate PDP sample packets.
...
def check_sephora_pdp_parser_fit(
    *,
    ...
    if capture_metadata.get("parser_version") != SEPHORA_PDP_PARSER_VERSION:
        raise ParserFitCheckError(
            ...
            f"stored parser version {capture_metadata.get('parser_version')!r} does not match current",
        )
    ...
    if not isinstance(profile, dict) or profile.get("name") != SEPHORA_PDP_CONTENT_PROFILE:
        raise ParserFitCheckError(
            "profile_mismatch",
            "browser metadata does not bind sephora_pdp_aggregate",
        )
    if browser_metadata.get("pin_confirmed") is not True:
        raise ParserFitCheckError(
            "source_mismatch",
            "browser metadata does not confirm the Sephora US/USD market pin",
        )
    ...
    try:
        stored_record = SephoraPdpAggregateContentRecord.model_validate(stored_payload)
    except Exception as exc:
        raise ParserFitCheckError(
            "content_record_shape", f"stored content record is invalid: {exc}"
        ) from exc
    if stored_record.record_kind != SEPHORA_PDP_CONTENT_RECORD_KIND:
        raise ParserFitCheckError(
            "content_record_shape",
            "stored content record kind is not Sephora aggregate PDP content",
        )
    if stored_record.parser_version != SEPHORA_PDP_PARSER_VERSION:
        raise ParserFitCheckError(
            "parser_version_mismatch",
            "stored content record parser version does not match current",
        )
    if stored_record.capture_profile != SEPHORA_PDP_CONTENT_PROFILE:
        raise ParserFitCheckError(
    ...
    current = build_sephora_pdp_aggregate_content_record(
    ...
    return {
        ...
        "current_parser_version": SEPHORA_PDP_PARSER_VERSION,
        "difference": None if match else _diff_summary(stored_payload, current),
    }


def run_sephora_pdp_parser_fit_check(
    *,
    ...
    for packet_path in packet_paths:
        try:
            result = check_sephora_pdp_parser_fit(
    ...
    return (
        1 if failed else 0,
        {
            "checker": "retail_pdp_sephora_aggregate_parser_fit_v1",
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check pinned Sephora aggregate PDP sample packet parser fit."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    exit_code, report = run_sephora_pdp_parser_fit_check(packet_paths=args.packet)
