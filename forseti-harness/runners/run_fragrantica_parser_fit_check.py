"""Parser-fit drift check for rendered Fragrantica sample packets.
...
def check_fragrantica_parser_fit(*, packet_or_manifest_path: Path) -> dict[str, Any]:
    manifest_path = (
    ...
    try:
        ...
        raise ParserFitCheckError("packet_shape", f"invalid packet manifest: {exc}") from exc
    if packet.source_family != SOURCE_FAMILY or packet.source_surface not in RENDERED_SURFACES:
    ...
    content_path = _verified_preserved_path(
        packet=packet, packet_dir=packet_dir, filename="content_record.json"
    )
    metadata_path = _verified_preserved_path(
        packet=packet, packet_dir=packet_dir, filename="content_capture_metadata.json"
    )
    metadata = _read_object(metadata_path, "content_capture_metadata_shape")
    if metadata.get("capture_artifact_mode") != "sample":
        raise ParserFitCheckError(
            "sample_required", "parser-fit checks require a sample packet"
        )
    if metadata.get("parser_version") != FRAGRANTICA_PARSER_VERSION:
        raise ParserFitCheckError(
            "parser_version_mismatch",
            f"stored parser version {metadata.get('parser_version')!r} does not match current",
        )
    ...
    for role in ("rendered_dom", "visible_text"):
        ...
        body = _verified_preserved_path(
            packet=packet, packet_dir=packet_dir, filename=filename
        ).read_bytes()
        if hashlib.sha256(body).hexdigest() != item.get("sha256"):
            raise ParserFitCheckError(
                "input_hash_mismatch", f"sample input {role!r} hash does not match metadata"
            )
    ...
    try:
        stored_record = FragranticaContentRecord.model_validate(stored_payload)
    except Exception as exc:
        raise ParserFitCheckError(
            "content_record_shape", f"stored content record is invalid: {exc}"
        ) from exc
    if stored_record.record_kind != FRAGRANTICA_CONTENT_RECORD_KIND:
        raise ParserFitCheckError(
            "content_record_shape", "stored content record kind is not Fragrantica rendered content"
        )
    if stored_record.parser_version != FRAGRANTICA_PARSER_VERSION:
        raise ParserFitCheckError(
            "parser_version_mismatch", "stored content record parser version does not match current"
        )
    current = build_fragrantica_content_record(
    ...
    return {
        ...
        "current_parser_version": FRAGRANTICA_PARSER_VERSION,
        "difference": None if match else _diff_summary(stored_payload, current),
    }


def run_fragrantica_parser_fit_check(
    *, packet_paths: Sequence[Path]
    ...
    for packet_path in packet_paths:
        try:
            result = check_fragrantica_parser_fit(
    ...
    return (
        1 if failed else 0,
        {
            "checker": "fragrantica_rendered_current_window_parser_fit_v1",
...
def _verified_preserved_path(
    *, packet: SourceCapturePacket, packet_dir: Path, filename: str
) -> Path:
    matches = [
        item
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Check rendered Fragrantica sample packet parser fit."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    exit_code, report = run_fragrantica_parser_fit_check(packet_paths=args.packet)
