"""Parser-fit drift check for pinned-route Basenotes sample packets.
...
def check_basenotes_parser_fit(*, packet_or_manifest_path: Path) -> dict[str, Any]:
    ...
    if packet.source_surface != SOURCE_SURFACE:
        raise ParserFitCheckError(
            "ineligible_source_surface",
            f"expected {SOURCE_SURFACE!r}, got {packet.source_surface!r}",
        )
    ...
    if metadata.get("parser_version") != BASENOTES_PARSER_VERSION:
        raise ParserFitCheckError(
            "parser_version_mismatch",
            f"stored parser version {metadata.get('parser_version')!r} does not match "
            f"current {BASENOTES_PARSER_VERSION!r}",
        )
    ...
    try:
        stored_record = BasenotesContentRecord.model_validate(stored_loaded)
    except Exception as exc:
        raise ParserFitCheckError(
            "content_record_shape",
            f"stored content record is invalid: {exc}",
        ) from exc
    if stored_record.record_kind != BASENOTES_CONTENT_RECORD_KIND:
        raise ParserFitCheckError(
            "content_record_shape",
            f"stored record kind is {stored_record.record_kind!r}",
        )
    if stored_record.parser_version != BASENOTES_PARSER_VERSION:
        raise ParserFitCheckError(
            "parser_version_mismatch",
    ...
    current_record = build_basenotes_content_record(
...
def run_basenotes_parser_fit_check(
    *,
    ...
    for packet_path in packet_paths:
        try:
            row = check_basenotes_parser_fit(packet_or_manifest_path=packet_path)
            if row["status"] != "match":
                failures += 1
    ...
    return (
        ...
        {
            "checker": "basenotes_current_window_parser_fit_v1",
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Check Basenotes sample packet parser fit.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    exit_code, report = run_basenotes_parser_fit_check(packet_paths=args.packet)
