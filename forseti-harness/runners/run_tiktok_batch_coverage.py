"""CLI: build a deterministic coverage view from an admitted TikTok batch packet."""
...
from source_capture.tiktok.batch_coverage import (  # noqa: E402
    build_tiktok_batch_coverage_from_lake,
    build_tiktok_batch_coverage_from_packet_directory,
    tiktok_batch_coverage_json_text,
    write_tiktok_batch_coverage,
)


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build a deterministic, text-free coverage view from an admitted TikTok batch packet."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.packet is not None:
            ...
            coverage = build_tiktok_batch_coverage_from_lake(data_root, args.packet_id)

        if args.output is not None:
            written = write_tiktok_batch_coverage(
                coverage=coverage,
        ...
        print(tiktok_batch_coverage_json_text(coverage), end="")
        return 0
