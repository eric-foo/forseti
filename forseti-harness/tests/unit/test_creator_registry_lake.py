...
def test_existing_v2_pointer_epoch_fails_closed_until_rebuild(tmp_path: Path) -> None:
    root = _root(tmp_path)
    migrated = _migrate(root)
    registry_root = root.path / "indexes" / "derived_retrieval" / "creator_registry"
    v3_generation = Path(migrated["generation_root"])
    ...
    # Stand in for a generation published by the v2 epoch: same generated bytes,
    # a v2 manifest, and a sibling generation id the v3 identity cannot produce.
    v2_generation_id = "crg_" + v3_manifest["authority_inventory_sha256"][:24]
    assert v2_generation_id != migrated["generation_id"]
    v2_generation = registry_root / "generations" / v2_generation_id
    (v2_generation / "query_tables").mkdir(parents=True)
    (v2_generation / "profiles").mkdir()
    (v2_generation / "manifests").mkdir()
    (v2_generation / "query_tables" / "creator_registry_index_v1.json").write_bytes(
        (v3_generation / "query_tables" / "creator_registry_index_v1.json").read_bytes()
    )
    (v2_generation / "profiles" / "creator_profile_public_v1.json").write_bytes(
        (v3_generation / "profiles" / "creator_profile_public_v1.json").read_bytes()
    )
    (v2_generation / "manifests" / "creator_registry_generation_v2.json").write_bytes(
        canonical_record_bytes(
            {
                **v3_manifest,
                "schema_version": "creator_registry_generation_v2",
                "generation_id": v2_generation_id,
            }
        )
    )
    v2_before = {
        path.relative_to(v2_generation).as_posix(): path.read_bytes()
        for path in v2_generation.rglob("*")
        if path.is_file()
    }
    (registry_root / "CURRENT").write_bytes(
        canonical_record_bytes(
            {"pointer_schema_version": 2, "generation_id": v2_generation_id}
        )
    )

    with pytest.raises(CreatorRegistryLakeError, match="pointer schema is unsupported"):
        load_current_creator_registry(root)
    with pytest.raises(CreatorRegistryLakeError, match="pointer schema is unsupported"):
        monitoring_eligible_accounts(root)

    rebuilt = publish_creator_registry_generation(root)
    assert rebuilt["generation_id"] == migrated["generation_id"]
    assert json.loads((registry_root / "CURRENT").read_text(encoding="utf-8")) == {
        "pointer_schema_version": 3,
        "generation_id": migrated["generation_id"],
    }
    assert load_current_creator_registry(root)["creator_registry_index"]["counts"][
        "platform_accounts_total"
    ] == migrated["platform_accounts_total"]
    assert v2_before == {
        path.relative_to(v2_generation).as_posix(): path.read_bytes()
        for path in v2_generation.rglob("*")
        if path.is_file()
...
def test_candidate_retraction_requires_explicit_v3_epoch_upgrade(tmp_path: Path) -> None:
    root = _root(tmp_path)
    _migrate(root)
    ...
    pointer.write_bytes(
        canonical_record_bytes({**pointer_doc, "pointer_schema_version": 2})
    )

    with pytest.raises(CreatorRegistryLakeError, match="rebuild --write"):
        retract_tiktok_creator_candidate(
            data_root=root, public_handle="new.fragrance"
        )
    assert not list(
...
def test_retracted_candidate_cannot_be_readmitted_or_validated(tmp_path: Path) -> None:
    root = _root(tmp_path)
    _migrate(root)
    packet_id, outcome_path, account_id = _packet_and_outcome(root)
    _candidate_admit(root, packet_id)
    ...
    # Reopening the Frontier disposition does not un-retract the account: the
    # retraction keeps verifying against disposition history.
    ...
    with pytest.raises(CreatorRegistryLakeError, match="retracted candidate account"):
        admit_tiktok_creator_candidate(
            data_root=root,
            packet_id=packet_id,
            frontier_disposition_id=disposition["disposition_id"],
    ...
    with pytest.raises(
        CreatorRegistryLakeError, match="exactly one Registry not_onboarded account"
    ):
        admit_tiktok_creator_account(
            data_root=root,
            packet_id=packet_id,
