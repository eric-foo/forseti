...
def test_product_public_handle_ledger_seed_loads_and_validates() -> None:
    ...
    account_ids = [
        account["platform_account_id"] for account in wrapper["platform_accounts"]
    ]
    ...
    assert {"ak.fragrances1", "calcologne", "mcnastyfragrance", "fragrance.chud"}.issubset(
    ...
    assert all(
        ...
        if account["public_handle"] not in {"mcnastyfragrance", "fragrance.chud"}
    )
    mcnasty = next(
    ...
    fragrance_chud = next(
        account for account in tiktok_accounts if account["public_handle"] == "fragrance.chud"
    )
    assert fragrance_chud["platform_public_account_id_or_none"] == "7628842689794049054"
