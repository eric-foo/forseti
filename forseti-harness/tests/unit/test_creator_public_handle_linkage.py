...
def test_product_public_handle_ledger_seed_loads_and_validates() -> None:
    ...
    assert len(wrapper["platform_accounts"]) == 38
    assert len(wrapper["account_link_evidence"]) == 1
    assert len(wrapper["creator_records"]) == 1
    ...
    assert [account["public_handle"] for account in ig_accounts] == ["hyram", "jeremyfragrance", "milanscents", "Fragranceknowledge"]
    ...
    assert [account["public_handle"] for account in tiktok_accounts] == ["funmimonet", "redolessence", "perfumerism", "fragranceknowledge"]
