# forseti-harness/youtube_capture
...
Requires `curl_cffi` — install via the extra: `pip install -e "forseti-harness[impersonate]"` (or
...
- **Manual freshness check** — CI deliberately does *not* do this (it would need a live third-party
  ...
  ```
  cd forseti-harness/youtube_capture
