...
open_next:
  - orca/product/spines/judgment/learning_loops/far_half/prospective_decision_loop_target_architecture_v0.md   # the LIVE shell (far half): decision-object, shadow/assisted, seal-before-disclose, decision memory
  - orca/product/spines/judgment/learning_loops/near_half/near_half_backtest_learning_architecture_v0.md        # the BACKTEST shell (near half): adversarial postmortem + validated-lesson cell + promotion gate
  - orca/product/spines/judgment/learning_loops/near_half/near_half_signal_reliability_ledger_v0.md             # the per-source weighting store (firewall-clean K-of-N report-all)
  - orca/product/spines/judgment/conductor/conductor_construction_integrity_probe_addendum_v1.md # current contamination doctrine (JSG-08 tell-audit; active recall dropped)
  - orca/product/spines/judgment/conductor/judgment_quality_promotion_operating_model_v0.md      # the FROZEN backtest conductor (core enters through the JSG-06 sealed judgment surface; never edit it)
  - orca/product/spines/foundation/demand_read_taxonomy/orca_demand_read_taxonomy_v0.md           # read grammar consumed by the core
  - orca/product/spines/scanning/scan_core/orca_demand_scan_core_spec_v0.md                       # scan observation schema feeding allowed demand signals
stale_if:
...
Cross-spine pointer: judgment consumes the taxonomy and scan emissions after
C1 allow/gate; it does not own scanning, source-class recon, or capture.
...
- `orca/product/spines/judgment/learning_loops/near_half/near_half_signal_reliability_ledger_v0.md` — the
...
- `orca/product/spines/product_lead/buyer_proof/orca_buyer_proof_packet_v0.md` — Demand-Substrate
  Hard Gate; action-ceiling vocab; no-scoring boundary; `product_learning` cap.
- `orca/product/spines/foundation/demand_read_taxonomy/orca_demand_read_taxonomy_v0.md` — signal layers,
