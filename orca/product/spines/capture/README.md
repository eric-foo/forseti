...
use_when:
  ...
  - Choosing between Capture product doctrine and orca-harness implementation files.
...
open_next:
  ...
  - orca/product/spines/capture/core/packet_schema
stale_if:
  - The Capture spine corpus migrates from orca/product/ into forseti/product/.
  - The Data Capture consolidation map is superseded.
  - The legacy orca-harness runtime root is migrated or replaced.
```
...
5. Open the operating model only when capture-lane workflow shape is
   load-bearing.
6. Inspect `orca-harness/` only after the product route shows implementation is
...
| Harness implementation | `orca-harness/**` | Legacy runtime files after implementation is explicitly authorized. |

## Boundary

This is a legacy physical path under `orca/product/`. It does not authorize live
capture, broad crawling, anti-blocking, API use, storage, dashboards,
production runtime, ECR, Cleaning, Judgment, or buyer proof. Use `orca-harness/`
