...
- ZGO remains the outlier: its public Yotpo widget endpoints returned zero
  reviews for the observed app key/product ids, while the Direct HTTP PDP static
  section still exposed the one captured row.
...
| `fragrance_retail_zgo` | ZGO Perfumery | `LOCKED_CAPTURE_FIXTURE_V0_STATIC_YOTPO_ONLY` | Direct HTTP on the known-reviewed fixture. Do not reuse the original zero-count Orpheon fixture for row capture. Public Yotpo widget endpoints are not row-positive for this fixture as probed. | `https://zgoperfumery.com/products/d-s-durga-concrete-lightning-eau-de-parfum` | Static HTML exposed a Yotpo review section with one review body, reviewer label, rating, and aggregate review count; date was not visible. Public Yotpo API calls returned zero rows/bottomline for the observed app key/product ids. | Use Direct HTTP static section only, with date and widget-mismatch residuals. | Re-probe if the static section disappears, count/body diverges, or a new Yotpo endpoint/app-key/product mapping is found. |
