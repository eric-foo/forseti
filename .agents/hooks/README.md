...
When you change a checker's behavior, run that checker's `--selftest` locally.
Required CI runs a `--selftest` only where `ci.yml` registers one (see `-List`
below), so it does not otherwise exercise changed checker fixtures. For a
`_hooklib.py` behavior change, also run the selftests of checkers that use
the changed helper.
...
It includes only CI-registered selftests and does not replace the changed-checker
selftests above. It excludes `python -m pytest` and `.github/scripts/*`;
