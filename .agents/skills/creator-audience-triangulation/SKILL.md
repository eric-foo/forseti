...
- A validation-blocked `queue-submit` leaves the job running. Return only its `validation_errors` to the same cold context and resubmit before the 30-minute lease expires. Use `queue-block` only for an explicit non-recoverable stop; otherwise let an abandoned lease expire for recovery. A claim that reports `recovered_succeeded` already found the exact validated outcome and completed Registry admission without another model call.
