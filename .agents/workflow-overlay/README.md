...
This overlay is the project authority for Forseti. Skills may provide task-local mechanics, but Forseti-specific facts and constraints must come from this folder or another Forseti-owned source named here. Some filenames and historical paths still carry the legacy Orca name until a separate rename/migration retires them.
...
Missing required project facts are `UNKNOWN - requires owner input`; do not fill them from `jb`, legacy Orca naming, or generic defaults.
