...
- Placement is checked in CI by `.agents/hooks/check_placement.py --changed --strict` (EP-04), which reads `repo-structure.yaml` as its only rule source, checks changed destinations against the exact event base, and fails visibly when strict diff scope is unavailable. The retained `--hook` mode is inactive compatibility. A passing check is placement shape only - never validation, readiness, or authority. Parameters and invariants: `docs/decisions/forseti_repo_structure_binding_v0.md`.
