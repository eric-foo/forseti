...
use_when:
  ...
  - Comparing repo-map, generated inventory, submap, recent-change, and gardener architecture options.
...
targets:
  ...
  - repo-map recent-change inbox and gardener loop
...
- Any generated-index, repo-structure, or map-link/checker source you can identify as directly decision-bearing; otherwise list it under `sources_available_not_read`.
...
Known current direction to test, not assume:
...
- Use generated inventory for complete file-level retrievability.
- Use area submaps for meaningful human navigation detail.
- Add a periodic gardener loop that promotes durable navigation changes into the main map/submaps.
...
Ask for the strongest source-backed case for the layered architecture:

- main map as curated router;
- generated inventory as complete file-level findability;
- area submaps as durable detail;
- recent-change inbox as low-conflict raw feed;
- gardener loop as periodic promotion.
...
- whether generated inventory creates false confidence or stale artifacts;
- whether gardener batching reintroduces conflict hotspots;
...
- what should remain judgment-based gardener work;
- the smallest next routing object if the architecture is accepted.
...
- AO-4 Layered map + generated inventory + gardener:
- AO-5 Other option if genuinely surfaced:
