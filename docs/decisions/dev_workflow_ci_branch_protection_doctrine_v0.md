...
direction_change_propagation:
  ...
  stale_language_search: >
    ...
    docs/decisions/dev_workflow_ci_branch_protection_doctrine_v0.md
    (run 2026-06-15 in the dev-workflow-uptodate-mgt worktree off origin/main @ 4302e7d7)
  stale_language_search_result: >
    Executed 2026-06-15. Item 9 already notes the in-session guard "lacks an up-to-date check (documented
    residual)"; item 12 is consistent with and acts on that note (downgrades the guard fix O2a, adopts
    bot-as-default instead). No live surface claims up-to-date is enforced on all paths, so item 12 forks
    no existing rule — it is additive.
  non_claims:
    - not validation, readiness, or acceptance of any lane's content
