...
- **Yield rates carry a prompt-filter confound.** The 208-output corpus was produced under find-time materiality-filtered prompts ("material decision-relevant failure modes"), so measured yield is recall-after-self-censoring, not recall. The bias is plausibly non-uniform across tiers — the materiality bar bites hardest on judgment/doctrine artifacts — so even the qualitative "high-lock-in is not higher-yield than code" claim may be partly a prompt artifact. Re-measure under coverage-first prompts (the 2026-07-04 review-lanes.md coverage-first find-stage doctrine) before treating yield comparisons as routing evidence; validating this confound is added to the cross-family arm's charge (§2/§7).
