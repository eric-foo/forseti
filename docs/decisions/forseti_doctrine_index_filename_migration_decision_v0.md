# Forseti Doctrine Index Filename Migration Decision v0

```yaml
retrieval_header_version: 1
artifact_role: Forseti decision record
scope: Bounded filename migration for the Doctrine Index router record.
use_when:
  - Resolving whether the live Doctrine Index router still uses a lowercase `orca_*` filename.
  - Auditing the family-by-family lowercase filename migration queue.
  - Updating live references to the Forseti Doctrine Index router.
authority_boundary: retrieval_only
open_next:
  - docs/migration/forseti_doctrine_index_filename_migration_v0/moved_paths_index.md
...
Migrate the live Doctrine Index router filename from lowercase `orca_*` to a
...
This batch is re-derived from current `origin/main` after PR #771 (`8584418b`,
including #753, #755, #756, #757, #758, #759, #760, #763, #765, #767, #768,
#769, #770, and #771). Older migration PRs or worktrees are reference material
only, not merge targets for this filename family.

| Old path | New path | Rule |
| --- | --- | --- |
| `docs/decisions/orca_doctrine_index_v0.md` | `docs/decisions/forseti_doctrine_index_v0.md` | Live Doctrine Index router; rename and update live references. |

## Compatibility Rule

Historical references to the old filename remain valid provenance and resolve
through `docs/migration/forseti_doctrine_index_filename_migration_v0/moved_paths_index.md`.
...
- Not a semantic amendment to any doctrine indexed by the Doctrine Index.
- Not creation of a `docs/doctrine/` folder or a change to doctrine/decision placement boundaries.
...
direction_change_propagation:
  doctrine_changed: >
    The Doctrine Index router now uses a Forseti filename for the live source,
    with the old path resolved by a moved-path index; remaining lowercase
  ...
  intentionally_not_updated:
    ...
    - path: old migration records describing the unmaterialized docs/doctrine folder decision
      reason: >
        Those records preserve the historical folder-boundary decision. This
        lane keeps the Doctrine Index in docs/decisions and changes only its
        filename.
    - path: remaining older Orca-named decision records
      reason: >
        Existing `docs/decisions/orca_*` settlement records require their own
        family-by-family classification. This lane updates the Doctrine Index
        successor and live references only.
  ...
  stale_language_search: >
    rg -n "docs/decisions/orca_doctrine_index_v0.md|orca_doctrine_index_v0.md"
    docs/decisions docs/workflows docs/migration forseti/product forseti-harness/docs
    --glob '!docs/prompts/**' --glob '!docs/review-inputs/**'
  ...
  non_claims:
    ...
    - not docs/doctrine folder creation
