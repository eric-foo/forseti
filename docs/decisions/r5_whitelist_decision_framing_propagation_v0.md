...
- **Finder frame** (`forseti/product/spines/foundation/vertical_exploration/forseti_memorization_resistant_case_finder_frame_v0.md`): :52-57 "swap on recognition" -> swap-on-outcome-USE + active recall dropped (already on the operative bar); anonymization bullet **enacted 2026-06-16** — replaced "optional flavor-anonymization, use sparingly" with **default-on identity-masking with sealed crosswalk** (mask target brand only; keep third-party identities / provenance / decision-numbers real; attributes-as-evidence; route-out; masking is a supplement, tell-audit unchanged); stays whitelist + R6 gate. **enacted**.
