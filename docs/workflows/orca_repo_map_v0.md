...
| `orca/product/spines/commission_signal_board/` | Live docs-only Commission Signal Board pilot spine. Use its README, spine manifest, playbook, prompt, authority packet, pointer docs, and moved-path index before CSB work. |
| `docs/decisions/orca_spine_first_workspace_structure_proposal_v0.md` | Partially accepted spine-first target: CSB pilot live under `orca/product/spines/commission_signal_board/`; global `orca/docs/` and other product-spine moves remain staged. Open before adding another top-level product workspace or moving spine-owned subsidiary documents. |
| `docs/migration/judgment_spine_spine_first_migration_inventory_v0.md` | Docs-only marking inventory for future Judgment Spine migration candidates under the proposed spine-first structure; does not move files or make `orca/` live. |
| `docs/migration/data_capture_projection_spine_first_migration_inventory_v0.md` | Docs-only marking inventory for raw-packet projection and projection-adjacent files; marks Mechanical Source Projection under the future Capture spine, not as a new Projection spine. |
| `docs/migration/capture_spine_ig_creator_migration_inventory_v0.md` | Docs-only marking inventory for future Capture Spine / IG creator-momentum migration candidates; separates direct creator-owned files from shared IG capture context and does not move files or make `orca/` live. |
...
| `orca-harness/runners/` | CLI entrypoints for case runs, memorization probe, bounded Candidate URL Intake live first-contact sourcing, Reddit Graph Frontier / crawling graph register preparation, source-capture packets (incl. the cross-archive `run_source_capture_historical_packet.py`, which runs the slice-E locate ladder and writes the selected rung's body plus a thin `archive_locate_metadata.json` ladder receipt), offline projection materialization such as `run_ig_creator_momentum_projection.py`, local Retail/PDP packet-directory projection (`run_retail_pdp_projection.py`; no capture, ECR, Cleaning, or Judgment), the opt-in Retail/PDP CloakBrowser projection sidecar (`run_source_capture_cloakbrowser_packet.py --source-family retail_pdp --retail-pdp-projection-output <path>`), and source-observability reports. Retail/PDP binding/residual implementation lives in `orca-harness/source_capture/retail_pdp_projection.py`. |
...
| `orca/product/spines/commission_signal_board/README.md` | Entry point for the live Commission Signal Board pilot spine and its non-goals. Open first for CSB spine work. |
| `orca/product/spines/commission_signal_board/spine.yaml` | Machine-readable CSB spine manifest: canonical artifacts, legacy stubs, shared validator/test surfaces, and non-goals. |
| `orca/product/spines/commission_signal_board/workflows/commission_signal_board_playbook_v0.md` | Operating playbook for the Commission Signal Board lane: intake first, full board generation, then manual validator run before any classifier-handoff safety claim. |
| `orca/product/spines/commission_signal_board/prompts/orca_commission_signal_board_prompt_v0.md` | Full prompt for producing a Commission Signal Board: evidence/signals-only source-family rows, graph-light retrieval brief, and demand-classifier handoff packet. Read with the playbook and validator. |
| `orca/product/spines/commission_signal_board/authority/orca_commission_signal_board_prompt_adjudication_packet_v0.md` | Decision-prep packet for why the temporary backtesting-first commission prompt became a durable signal-board prompt; keeps commission evidence/signals-only and separates it from demand classification, proof, and runtime work. |
| `orca/product/spines/commission_signal_board/migrations/moved_paths_index.md` | Active moved-path index for old CSB paths under `docs/`; also records validator/test surfaces intentionally left global. |
| `docs/decisions/orca_spine_first_workspace_structure_proposal_v0.md` | Partially accepted structure target for whole-spine workspaces outside `docs/product`; CSB is live, global `orca/docs/` and other product-spine moves remain staged. |
| `docs/prompts/handoffs/commission_signal_board_spine_pilot_reconciliation_handoff_prompt_v0.md` | Handoff prompt for the Commission Signal Board lane to reconcile stale "commission gate" language, inventory related artifacts, and plan or execute the Commission Signal Board spine pilot under the proposed structure boundary. |
| `docs/migration/commission_signal_board_spine_pilot_migration_plan_v0.md` | Applied Commission Signal Board spine-pilot migration plan: records the artifact inventory, old-path stubs, moved-path index, live CSB spine boundary, and staged status of global docs/other-spine moves. |
| `docs/migration/judgment_spine_spine_first_migration_inventory_v0.md` | Judgment Spine migration-marking inventory for the proposed spine-first structure. Open before creating a Judgment move manifest or relocating Judgment docs. |
| `docs/migration/data_capture_projection_spine_first_migration_inventory_v0.md` | Data Capture projection migration-marking inventory for the proposed spine-first structure. Open before creating a Capture/projection move manifest or deciding whether projection gets its own spine. |
| `docs/migration/capture_spine_ig_creator_migration_inventory_v0.md` | Capture Spine / IG creator-momentum migration-marking inventory. Open before creating a creator-momentum move manifest or relocating creator-momentum docs, and do not treat it as a broader IG migration. |
...
| `docs/migration/judgment_spine_spine_first_migration_inventory_v0.md` | Migration-marking inventory for future spine-first Judgment allocation. Use only for planning a later manifest; it is not a move authorization. |
...
| `docs/migration/data_capture_projection_spine_first_migration_inventory_v0.md` | Docs-only migration mark for projection-related Capture files. Records the current verdict that Mechanical Source Projection belongs under Capture, with Core/Cleaning/ECR consumers kept adjacent/shared. |
| `docs/migration/capture_spine_ig_creator_migration_inventory_v0.md` | Docs-only migration mark for the Capture Spine / IG creator-momentum lane. Use it to separate direct creator-momentum files from adjacent shared IG capture files before any later move manifest. |
| `docs/product/data_capture_spine/data_capture_spine_corpus_intake_obligation_contract_proposal_v0.md` | **Corpus Intake (standing-capture) obligation contract — owner-ratified 2026-06-15.** The standing sibling of the v0 commissioned obligation contract: the obligation home for recurring capture of an approved public signal into an append-only corpus before a Decision Frame. Ratified, not pressure-tested; authorizes no build, scheduler, runtime, or source access. |
