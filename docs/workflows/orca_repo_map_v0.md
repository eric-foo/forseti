...
| `docs/product/core_spine/core_spine_v0_data_lake_storage_contract_v0.md` | Data Lake Storage Contract v0: non-selecting storage contract for Raw Packet Store, Attachment Record, Availability Index, Derived Result Store, and Acknowledgement Log; records the blocker-1 Attachment Record direction as manifest-indexed immutable attachment bodies while keeping by-key discovery authoritative and deferring exact sidecar/member layout, serialization, backend, queue, schema, migration, validation, and readiness choices. |
