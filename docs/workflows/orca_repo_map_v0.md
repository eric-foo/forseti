...
| `docs/product/core_spine/core_spine_v0_data_lake_core_contract_v0.md` | Data Lake Core Contract v0: lake-owned raw packet preservation, by-key findability, content-free availability facts, source payload attachment boundary, and logical append-only result attachment; excludes Cleaning, ECR/SCR interpretation, Judgment, orchestration, queue authority, physical storage selection, and domain ontology from lake core. |
| `docs/product/core_spine/core_spine_v0_data_lake_storage_contract_v0.md` | Data Lake Storage Contract v0: non-selecting storage contract for Raw Packet Store, Attachment Record, Availability Index, Derived Result Store, and Acknowledgement Log; records blocker 1 as manifest-indexed immutable Attachment Record bodies and blocker 2 as legacy-readable transitional incumbent direct fields with future dual-read/replay only, while keeping by-key discovery authoritative and deferring exact sidecar/member layout, serialization, backend, queue, schema, migration, validation, and readiness choices. |
| `docs/product/core_spine/core_spine_v0_data_lake_attachment_record_implementation_contract_v0.md` | Data Lake Attachment Record Implementation Contract v0: implementation-facing blocker-1 contract for manifest/index entries linked to immutable/checkable attachment bodies; preserves no-smart-lake, no-backend-selection, no-sidecar-selection, and no-runtime-readiness boundaries for future storage implementation scoping. |
| `docs/product/core_spine/core_spine_v0_data_lake_mechanics_map_v0.md` | Planning-only logical mechanics map for raw capture through projection, ECR/SCR, Cleaning, and Judgment by key; use the Data Lake Core Contract for the current lake-owned responsibility boundary. |
