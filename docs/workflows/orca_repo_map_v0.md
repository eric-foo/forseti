...
| `orca/product/spines/data_lake/workflows/core_spine_v0_data_lake_bronze_full_gt_gate1_attachment_record_body_layout_adr_v0.md` | Gate 1 ADR awaiting ratification: packet-member selected as the default Attachment Record body home with the eight required outputs; attachments/ sidecar reserved behind a reopen trigger; external bodies rejected until Gate 2; entry serialization stays deferred to A2 behind A1. |
| `orca/product/spines/data_lake/workflows/core_spine_v0_data_lake_bronze_full_gt_gate2_retention_lawful_erasure_posture_adr_v0.md` | Gate 2 ADR awaiting ratification: lawful erasure explicitly deferred with the full accepted-residual record - residual scope, claim ceiling, tombstone/supersession unavailability posture, forbidden backend classes/operations while deferred, and four revisit triggers. |
...
| `orca/product/spines/data_lake/workflows/core_spine_v0_data_lake_consumption_seam_scoping_route_v0.md` | Consumption seam v0 scoping route awaiting owner acceptance: non-executing STEP-01..06 route for the shared pickup/ack helper contract, the derived_retrieval rebuild builder for the three gate-opened views, and the on-demand-first metrics policy with metric families operator_to_fill; implementation stays blocked on explicit bounded owner authorization. |
...
| `orca-harness/tests/` | `unit/`, `contract/`, and `integration/` tests plus fixtures, including no-LLM-import, no-tools contract guards, Fragrantica projection/Cleaning/lake tests, data-lake doctor inspection tests, Bronze Catalog rebuild/inspect/stale/orphan tests, Commission Signal Board validator fixtures, CSB-first scanning artifact checker fixtures, creator public-handle linkage synthetic/adversarial fixtures, and TikTok SCI single-video, parsed-batch admission, admitted-batch coverage/projection, and live batch probe parser/runner tests. |
