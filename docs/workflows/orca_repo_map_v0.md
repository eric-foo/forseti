...
- Latest focused addition: 2026-07-01 (`run_tiktok_batch_projection.py` plus `source_capture/tiktok/batch_projection.py` build a deterministic, text-free mechanical projection over admitted TikTok batch coverage rows from packet directories or verified data-lake packet ids; rows are anchored to preserved batch JSON pointers and carry counts, postures, hashes, and residuals only, with no product extraction, Cleaning, ECR, Judgment, or persisted derived lane). Prior: 2026-07-01 (`run_tiktok_batch_coverage.py` plus `source_capture/tiktok/batch_coverage.py` build a deterministic, text-free admitted TikTok batch coverage view from packet directories or verified data-lake packet ids; focused tests cover counts, lake-by-id CLI, stdout, raw text exclusion, bad input, and forbidden imports).
