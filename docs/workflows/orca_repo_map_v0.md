...
| `orca/product/spines/data_lake/workflows/core_spine_v0_data_lake_bronze_full_gt_physicalization_proof_scoping_route_v0.md` | Post-ratification read-only implementation-scoping route for the physicalization proof scope: A1 deterministic inventory gate plus the six-invariant proof gate (write-once, append-only, read-by-key, hash verification, public AR resolution, index rebuild) under the ratified packet-member relationship; STEP-01..06; ends at a single owner yes/no on bounded implementation; not implementation, A2/backend selection, or a full-GT claim. |
