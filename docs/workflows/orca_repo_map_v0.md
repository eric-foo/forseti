...
| `docs/product/core_spine/core_spine_v0_data_lake_storage_contract_v0.md` | Data Lake Storage Contract v0: non-selecting storage contract for Raw Packet Store, Attachment Record, Availability Index, Derived Result Store, and Acknowledgement Log; keeps by-key discovery authoritative and defers physical storage, manifest, sidecar, serialization, queue, schema, migration, validation, and readiness choices. |
