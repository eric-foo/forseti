...
| `orca-harness/runners/run_source_capture_cloakbrowser_profile_warmup.py` | Direct no-Playwright/CDP CloakBrowser profile warmup for permitted manual login continuity; creates/reuses only a label-indirected ignored `_browser_user_data/` directory, writes no packet, and saves no auth-state by itself. |
