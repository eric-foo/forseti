...
| `docs/decisions/orca_spine_first_workspace_structure_proposal_v0.md` | Proposed, non-binding mini-god-tier target for moving Orca to `orca/product/spines/<spine>/` product workspaces plus future `orca/docs/` global docs, with Commission Signal Board as the pilot. Open before adding a new top-level product workspace or moving spine-owned subsidiary documents. |
...
| `docs/decisions/orca_spine_first_workspace_structure_proposal_v0.md` | Proposed structure target for whole-spine workspaces outside `docs/product`, separate future `orca/docs/` global docs, subsidiary document allocation, and Commission Signal Board as the pilot. Non-binding until accepted by a later repo-structure decision. |
...
| `docs/migration/commission_signal_board_spine_pilot_migration_plan_v0.md` | Docs-only Commission Signal Board spine-pilot migration plan: inventories current artifacts, proposes the future spine layout, records why live `orca/` creation is blocked, and names the binding surfaces required before any file move. |
