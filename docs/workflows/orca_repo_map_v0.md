...
| `orca-harness/capture_spine/` | Capture-spine planning/validation helpers: Reddit Graph Frontier, LinkedIn lane/frontier validators, `creator_public_handle_linkage/` for the static public-handle linkage ledger validator, and `youtube_creator_observation/` for the YouTube creator observation ledger rebuild/live-lake verifier. Navigation context only; not live capture, runtime readiness, SQLite adoption, metric rollups, or person identity proof. |
...
| `orca-harness/tests/` | `unit/`, `contract/`, and `integration/` tests plus fixtures, including no-LLM-import, no-tools contract guards, Fragrantica projection/Cleaning/lake tests, Commission Signal Board validator fixtures, CSB-first scanning artifact checker fixtures, and creator public-handle linkage synthetic/adversarial fixtures. |
