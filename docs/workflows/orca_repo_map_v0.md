...
| `orca/product/spines/data_lake/workflows/core_spine_v0_data_lake_a2_attachment_record_entry_serialization_adr_v0.md` | A2 ADR awaiting ratification: A2-F2 packet index selected pending owner yes — the durable canonical object is the versioned entry schema plus deterministic derivation rule, never a materialized row; Manifest v2 reserved behind revisit triggers T-A2-1..3; hybrid-forever and packet-member AR file rejected; attachment_record_id stays a query locator; consumes the A1 inventory fork tags and an adjudicated cross-vendor consultation; not implementation, backend (Gate 2 T3), retention, or full-GT. |
