...
**CSB-first scanning artifact checker (portable/manual).** `.agents/hooks/check_csb_scanning_artifact.py`
...
overclaim leakage. It is not wired as an automatic PostToolUse hook. It is run
explicitly when producing or reviewing a CSB-first scan artifact. Fixtures live
under `orca-harness/tests/fixtures/csb_scanning_artifacts/`; focused tests live
at `orca-harness/tests/unit/test_csb_scanning_artifact_validator.py`. A pass is
...
| `orca-harness/tests/` | `unit/`, `contract/`, and `integration/` tests plus fixtures, including no-LLM-import, no-tools contract guards, Commission Signal Board validator fixtures, CSB-first scanning artifact checker fixtures, and creator public-handle linkage synthetic/adversarial fixtures. |
