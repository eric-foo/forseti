...
| `docs/decisions/forseti_external_identity_path_migration_decision_v0.md` | Post-audit migration decision splitting external repo/local identity from internal compatibility path/package/CI/skill migrations; recommends owner-gated repo/local identity cutover first and defers roots/packages/check names until moved-path validation exists. |
...
| `docs/research/aphrodite_depth_rehearsal_corpus_v0.md` | Frozen input corpus of the Aphrodite single-creator depth rehearsal (12 attributed jeremyfragrance IG reels with verified capture provenance); resolves the rehearsal claims' source_refs. |
| `docs/research/aphrodite_depth_rehearsal_ontology_slice_v0.md` | Hand-built minimal fragrance ontology slice for the depth rehearsal (receipt-backed houses/products/scene vocabulary; not the ontology build). |
| `docs/research/aphrodite_depth_rehearsal_extraction_recipe_v0.md` | Versioned rehearsal extraction recipe (`aphrodite-rehearsal-extraction-v0`): claim types, provenance fields, hashing and abstention rules; hand-run precursor of the depth-layer extractor. |
| `docs/research/aphrodite_depth_rehearsal_derived_claims_v0.json` | Rehearsal derived-claim set (19 claims, 7-field derivation provenance per the Aphrodite provenance contract; withhold-not-zero-fill). |
| `docs/research/aphrodite_depth_rehearsal_fit_panel_v0.md` | Rehearsal rendering of the fit evidence panel for jeremyfragrance (2 of 5 components withheld; not decision-grade by design). |
| `docs/research/aphrodite_depth_rehearsal_ad_reception_panel_v0.md` | Rehearsal rendering of the sponsorship-load + ad-reception panel for jeremyfragrance (core sponsored-vs-organic comparison honestly downgraded). |
| `docs/research/aphrodite_depth_rehearsal_grade_v0.md` | Honest grade of the depth rehearsal against charter panel definitions plus the sized build shopping list; advisory input to charter D-1. |
| `docs/research/aphrodite_depth_rehearsal_round2_gentsscents_grade_v0.md` | Round-2 depth rehearsal on GentsScents (YouTube long-form): substrate delta vs the Jeremy corpus and a re-grade under the owner's time-excluded rubric (fit + ad-reception D+/C- → A-) proving round-1 caps were substrate-bound; ad-reception is extraction over already-captured description/metadata, not a capture gap. |
| `docs/research/aphrodite_depth_rehearsal_round2_share_of_voice_v0.md` | Share-of-voice rehearsal over the 15 GentsScents transcripts (417 assisted-extracted product mentions): editorial vs attention-weighted SoV and the empirical finding that the two converge over a narrow-view-spread window — attention-weighting's value is unlocked by capturing across the performance range, not by top-slicing. Rehearsal artifact, not a silver-lane record. |
