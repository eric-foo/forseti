...
| `orca-harness/source_capture/` | Source-capture packet core: models, writer, CLI support, plaintext receipts, the `block_shell` honest-success classifier (block-shell / empty / content-unverified; no positive content class), and the bounded Reddit screening-read entry `screening_reddit_read.py` (entitlement-gated, screen-light, adapter-not-runner: wires `fetch_direct_http_capture` only, never the packet/ECR runner; returns a screen-light record, no disk artifact / no ECR). |
