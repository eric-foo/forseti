...
1. **Decision set.** Propose the concrete decisions Phase 2 will resolve,
   each named by reader, choice, and horizon. The two candidate frames from
   the GTM contract: the defensive subject pitch (Tower 28 itself as reader)
   and the offensive indie-buyer read (a competing/adjacent indie as reader).
