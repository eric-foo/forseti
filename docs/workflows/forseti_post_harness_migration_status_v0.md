...
Baseline census observed from `origin/main` after PR #675 merged as `c10f1d7f` on 2026-07-05. Later rows below record subsequent executed migration lanes where explicitly cited. Current convergence planning treats `origin/main` after PR #758 (`650e0e81`, including #753, #755, #756, #757, and #758) as the baseline; pre-#753/#755/#756 migration branches are spent unless their remaining value is manually re-derived against current `main`.
...
| Current-main convergence | Executed through PR #758: #753 recorded the local checkout split, #755 landed ontology GT ladder/A1/A2/R0 SSOT fixes, #756 landed the TikTok packet-grade runner posture on `forseti-harness/`, #757 aligned the migration plan to current `main`, and #758 migrated the ontology filename family. | Future migration and conflict-resolution lanes start from current `main`; do not replay stale side-branch trees over #755/#756/#758 files. |
...
| Product-lead authority filename family | In progress in the bounded product-lead authority filename migration lane: the live product thesis, offer hypothesis, buyer-proof packet, product-proof charter, and claim-defense doctrine filenames move to Forseti names. | Historical old filenames resolve through `docs/migration/forseti_product_lead_authority_filename_migration_v0/moved_paths_index.md`; remaining lowercase `orca_*` filename families stay separate queued batches. |
...
| Lowercase `orca_*` filenames | Partially deferred: the ontology and product-lead authority filename families have migrated; other families remain queued by default. | Migrate only by family with moved-path/index coverage; do not word-match historical prompts, reviews, receipts, research snapshots, or source-hash rows. |
...
Migration branches or handoffs authored before PR #753, PR #755, PR #756, or PR #758 are
...
authority filenames, and TikTok runner/harness files that changed in those PRs.
...
alignment, and #758 ontology filename migration have landed, the remaining
material lanes are:
...
3. Continue family-by-family live filename migration for current product sources
   whose `orca_*` filenames are still operator-facing; the ontology filename
   family has landed, this lane handles the product-lead authority filename
   family, and remaining families still need moved-path coverage plus a fresh
   read of any #755/#756/#758-touched source before editing.
