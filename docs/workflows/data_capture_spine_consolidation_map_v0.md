...
| Check YouTube Shorts creator/channel observations from the 200-row fragrance pool | `orca/product/spines/capture/core/source_families/social_media/youtube/youtube_creator_observation_ledger_spec_v0.md`, `orca/product/spines/capture/core/source_families/social_media/youtube/youtube_shorts_fragrance_creator_observation_ledger_v0.json`, and `orca-harness/tests/unit/test_youtube_creator_observation_ledger.py` (source-backed YouTube-only observation ledger; no cross-platform linkage, no metric rollups, no transcript bodies, no SQLite) |
