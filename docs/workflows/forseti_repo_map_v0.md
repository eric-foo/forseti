...
| `forseti-harness/runners/run_tiktok_audience_evidence_extract.py` | Owner-gated, injected-transport TikTok subtitle audience-evidence runner; token-budget batches transcripts across creators while persisting and completing per transcript, verifies exact source quotes, keeps addressed-audience and beneficiary/content-fit evidence separate, and fixes actual audience to `not_estimated`. |
