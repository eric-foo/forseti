...
| `docs/workflows/forseti_post_harness_migration_status_v0.md` | Current Forseti migration status ledger: product root, repo-map path, harness root, package label, CI check, GitHub repo slug, product-lead skill ID, and fresh local `projects/forseti` clone are migrated; legacy active `projects/orca` workspace closeout, start-preflight alias, and lowercase filename families remain separate migration units. |
