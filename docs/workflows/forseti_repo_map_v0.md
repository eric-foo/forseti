...
| `docs/decisions/forseti_skill_preflight_identity_migration_plan_v0.md` | Skill/preflight identity migration plan: keeps `forseti_start_preflight` primary, preserves `orca_start_preflight` as a legacy alias, and defers `orca-product-lead` retirement until a skill-governance lane introduces `forseti-product-lead` with compatibility, deployment, resolver, hash-pin, and rollback handling. |
| `docs/decisions/forseti_external_identity_path_migration_decision_v0.md` | Post-audit migration decision splitting external repo/local identity from internal compatibility path/package/CI/skill migrations; records the current `eric-foo/Forseti` target-slug blocker and defers repo/remotes/protected-action cutover until the owner frees that slug or chooses an alternate. |
