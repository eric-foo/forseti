...
| `docs/workflows/forseti_post_harness_migration_status_v0.md` | Current post-PR-675 Forseti migration status ledger: product root, repo-map path, harness root, package label, and CI check are migrated; external repo slug, local parent folder, skill ID, start-preflight alias, and lowercase filename families remain separate migration units. |
