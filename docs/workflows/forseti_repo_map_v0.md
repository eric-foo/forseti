...
| `docs/decisions/forseti_harness_identity_migration_plan_v0.md` | Phased migration plan for retiring the `orca-harness/` runtime root, `orca-harness` distribution label, and `orca-harness-tests` CI check as compatibility identifiers without changing Python import namespaces or historical provenance. |
