...
| `docs/workflows/forseti_data_lake_physical_rename_runbook_v0.md` | Owner-gated runbook for the physical data-lake root rename `F:\orca-data-lake` -> `F:\forseti-data-lake`: verified baseline, quiesce/preflight gates, marker promotion preserving the root UUID, env-pointer migration, legacy-marker retirement, tombstone, fail-closed verification, and rollback map. Plan awaiting explicit owner approval; not execution, validation, or lake-health proof. |
