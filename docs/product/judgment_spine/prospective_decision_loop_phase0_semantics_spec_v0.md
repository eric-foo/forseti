...
artifact_role: PROPOSED Phase-0 semantics spec (consumes the PROPOSED target architecture on PR #34; pending owner landing/ratification of both — binds nothing, builds nothing, authorizes no seal)
scope: >
...
branch_or_commit: prospective-loop-phase0-semantics-v0 off bac084f (stacked on PR #34)
stale_if:
...
`PROPOSED` — Phase-0 docs contract, product-learning tier, stacked on a PROPOSED
target. Nothing here is runnable as evidence until (a) the owner lands/ratifies
the target and (b) signs the Phase-1 pilot ledger. The worked example below is
