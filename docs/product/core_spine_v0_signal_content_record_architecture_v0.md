...
- **Common spine (family-invariant):** `content_id` · `signal_family` (closed enum: the five families + `RESIDUAL_FAMILY_UNRESOLVED`) · `subject_entity` (who/what the signal is *about* — not the source) · `event_or_claim` (the observed substance, raw, pre-interpretation) · `signal_event_time` (by reference to `PacketTiming`, distinct from capture time and cutoff) · `decision_relevance` (the neutral tag, #3) · `raw_observation` (verbatim, source-language anchor).
