...
scope: >
  ...
  Foundation Layer heavyweight discovery by inverting its fame bias. States requirements
...
use_when:
  ...
  - Extending or reviewing Foundation Layer heavyweight discovery for memorization-resistance.
...
open_next:
  ...
  - docs/product/foundation/orca_vertical_exploration_guide_v0.md   # WHERE-side vertical exploration guide (owner-adopted shape, subordinate to this frame)
stale_if:
  ...
  - Foundation Layer discovery eligibility / anti-leakage doctrine changes.
```
...
`docs/product/foundation/`; the original "untracked, durable-home
...
*extends* Foundation Layer heavyweight discovery; it does not replace or rebuild it.
...
same boundaries Foundation Layer discovery and the data-capture thesis already draw.
...
## Connection to Foundation Layer discovery
...
- **Upstream context:** Foundation Layer heavyweight discovery (extended here).
