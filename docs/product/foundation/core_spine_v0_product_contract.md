# Foundation Layer v0 Product Contract

- Status: PROPOSED_FREEZE
- Artifact type: Product artifact
- Scope: Market-agnostic Foundation Layer contract for Orca v0
...
Foundation Layer v0 is Orca's reusable decision-evidence foundation. It turns public market
...
Foundation Layer owns market-agnostic evidence mechanics. Satellites own
...
## Frozen Foundation Primitives

Foundation Layer v0 is intentionally compressed into eight primitives. The product
should not add a standalone axis when the concern can be absorbed into one of
these primitives.

| Priority | Primitive | Foundation responsibility | Absorbs |
| --- | --- | --- | --- |
...
Foundation Layer v0 requires only these product objects:
...
Raw public market signals are not final evidence. Foundation Layer must clean,
...
The Foundation Layer is the repeatable evidence foundation. Satellites adapt it to buyer,
...
Do not promote these into the Foundation Layer:
...
| Source-quality and engagement evidence standard | 25% | Tests the reusable differentiator. |
| Foundation Layer contract | 15% | Establishes the reusable boundary. |
...
Backtesting is central. It should be used to sharpen the evidence standard
...
Foundation Layer v0 does not authorize:
...
until the Foundation Layer contract, evidence standard, Client 0 proof plan,
