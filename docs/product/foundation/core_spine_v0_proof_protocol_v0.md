# Foundation Layer v0 Proof Protocol

- Status: PROPOSED_FREEZE
- Artifact type: Product proof protocol
- Scope: Docs-first proof plan for Foundation Layer v0
...
This protocol defines what proof is sufficient to show that Foundation Layer v0 can
...
- Foundation Layer v0 is usable for decision-grade public-signal interpretation.
- Foundation Layer v0 can support more than one satellite without adding new core
  primitives.
...
Foundation Layer v0 proof requires all of the following:

1. A Client 0 `jb` decision memo dry run.
2. One materially different non-`jb` shadow satellite decision memo dry run.
3. A shared evidence standard based on the eight Foundation Layer primitives.
...
## Shared Foundation Layer Primitives
...
templates, paths, or validation habits into Foundation Layer authority.
...
The shadow satellite passes portability only if it uses the same Foundation Layer
...
- Both memos use the same eight Foundation Layer primitives.
...
Blocked is not a failure of Foundation Layer v0. It means the proof cannot be judged
