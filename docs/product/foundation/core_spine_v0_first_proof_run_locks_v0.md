# Foundation Layer v0 First Proof Run Locks

- Status: LOCKS_REVIEWED_AWAITING_PROOF_RUN_AUTHORIZATION
- Artifact type: Product proof-run lock record
- Scope: First Foundation Layer v0 proof-run starting conditions before evidence collection or interpretation
...
- This record does not import `jb` project paths, lifecycle mechanics,
  templates, validation habits, or success logic into Orca or Foundation Layer
  authority.
...
- Underclaiming valid costly behavior would make the method less useful than
  Foundation Layer v0 is meant to be.
...
- `Commit`: allowed only if later evidence satisfies the full Foundation Layer
...
success logic do not become Orca or Foundation Layer authority. `jb` can test method
...
- The case requires new Foundation Layer primitives.
...
It must use the same Foundation Layer primitives used by the `jb` and shadow
...
- `jb` authority leaks into Orca or Foundation Layer authority;
- recommendation verbs exceed the evidence-supported Action Ceiling.
