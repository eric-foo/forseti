...
open_next:
  - docs/product/source_capture_toolbox/README.md                                   # the Armory (rendered adapters)
  - docs/product/source_capture_toolbox/capture_recon_index_v0.md                   # Sephora/Ulta recon (worktree, pending-merge); no Amazon recon
  - docs/product/source_capture_toolbox/source_capture_playbook_v0.md               # the capture PLAYBOOK (probe-then-pin, recon procedure) — USE IT to assess each retailer
  - docs/product/source_capture_toolbox/source_capture_anti_block_ladder_usage_guide_v0.md   # anti-block ladder (direct_http -> anti_blocking_http -> visible browser -> cloakbrowser+proxy)
  ...
  - docs/product/data_capture_spine/core_spine_v0_data_capture_spine_obligation_contract_v0.md   # Ob.6 fidelity, Ob.17 durability facts
...
1. **Same packet shape, per retailer.** Each authorized retailer is captured into the **same `SourceCapturePacket`** the `direct_http` path produces — the Ob.6 fidelity dimensions and the Ob.17 durability fields (Element 1 pins, Element 2 series origin, Element 4 cadence) are populated as first-class schema fields, so the cadence runner treats a rendered observation identically to a `direct_http` one.
