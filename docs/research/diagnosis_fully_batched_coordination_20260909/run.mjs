...
for(let round=0;round<=2;round++){
 ...
 const executable='C:/Users/vmon7/AppData/Local/OpenAI/Codex/bin/fd4c151a749f3ab4/codex.exe';
 const args=['exec','--json','--model','gpt-6-astra','-c','model_reasoning_effort="medium"','-C',path.join(dir,callBase),'--disable','shell_tool','--output-schema',path.join(dir,'response.schema.json'),'--output-last-message',path.join(dir,callBase,'response.json'),'-'];
 const started_at=new Date().toISOString();const clock=performance.now();
 await write(callBase+'/invocation.json',{executable,args,cwd:path.join(dir,callBase),started_at,stdin_sha256:sha(prompt),no_resume:true,model_override:"gpt-6-astra",reasoning_override:"medium",retrieval_round:round});
